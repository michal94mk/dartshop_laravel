...
@section('title', 'Dodaj poradnik')

@section('content')
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-gray-800">Dodaj nowy poradnik</h2>
                <p class="mt-1 text-sm text-gray-600">Utwórz nowy poradnik dla użytkowników.</p>
            </div>
            ...
            <form action="{{ route('admin.tutorials.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <p class="mt-1 text-xs text-gray-500">Jeśli pozostawisz puste, slug zostanie wygenerowany automatycznie z tytułu.</p>
                    @error('slug')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="content" id="content" rows="15" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="mb-4">
                        ...
                        <input type="text" name="category" id="category" value="{{ old('category') }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        @error('category')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="difficulty" id="difficulty" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            <option value="beginner" {{ old('difficulty') == 'beginner' ? 'selected' : '' }}>Początkujący</option>
                            <option value="intermediate" {{ old('difficulty') == 'intermediate' ? 'selected' : '' }}>Średniozaawansowany</option>
                            <option value="advanced" {{ old('difficulty') == 'advanced' ? 'selected' : '' }}>Zaawansowany</option>
                        </select>
                        @error('difficulty')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <input type="datetime-local" name="published_at" id="published_at" value="{{ old('published_at') }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="meta_description" id="meta_description" rows="2" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('meta_description') }}</textarea>
                ...
                <div class="mb-4">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_published" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" {{ old('is_published') ? 'checked' : '' }}>
